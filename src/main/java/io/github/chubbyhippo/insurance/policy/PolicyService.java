...
public class PolicyService {
    ...
    public PolicyService(PolicyRepository repository, UnderwritingService underwriting, ApplicationEventPublisher events) {
    ...
    @Transactional
    public UUID createDraft(String customerId, String coverageType, int coverageLimit, LocalDate start, LocalDate end) {
    ...
    public void activate(UUID policyId) {
        Policy policy = repository.findById(policyId).orElseThrow(() -> new IllegalArgumentException("Policy not found"));
        policy.activate();
        repository.save(policy);
    ...
    public Policy get(UUID id) {
        return repository.findById(id).orElseThrow(() -> new IllegalArgumentException("Policy not found"));
    }
}
