...
public class InvoiceService {
  ...
  public void createInvoiceFor(UUID policyId) {
    ...
    var alreadyOpen = invoices.existsByPolicyIdAndStatus(policyId, InvoiceStatus.OPEN);
  ...
  public void voidOpenInvoicesFor(UUID policyId) {
    ...
    var open = invoices.findByPolicyIdAndStatus(policyId, InvoiceStatus.OPEN);
    if (open.isEmpty()) return;

    var now = Instant.now(clock);
    for (var invoice : open) {
      invoice.voidNow(now);
    }
